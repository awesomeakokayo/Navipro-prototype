      ...
      if (month.weeks && month.weeks.length > 0) {
        month.weeks.forEach((week, weekIndex) => {
          const weekElement = createWeekElement(
            ...
            monthIndex + 1,
            weekIndex + 1,
            roadmapData // Pass the full roadmap data to check completion status
          );
...
function createWeekElement(week, monthNumber, weekNumber, roadmapData) {
...
function setupProgressTracking(userId, roadmapData) {
  ...
  taskItems.forEach((taskItem) => {
    taskItem.addEventListener("click", async function () {
      ...
      try {
        // First, mark the task as completed in the backend
        const response = await fetch(
