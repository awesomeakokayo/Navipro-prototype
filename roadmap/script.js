...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    try {
        ...
        // Save latest roadmap in localStorage
        localStorage.setItem("roadmapData", JSON.stringify(roadmapData));

        // Update UI
        updateRoadmapDisplay(roadmapData);
        setupProgressTracking(result.user_id || userId);
    } catch (err) {
        console.error("Failed to fetch roadmap:", err);

        // Fall back to cached roadmap if available
        const cached = JSON.parse(localStorage.getItem("roadmapData") || "{}");
        if (cached.roadmap) {
            updateRoadmapDisplay(cached);
    ...
    if (isOddWeek) {
        weekDiv.innerHTML = `
            <div class="week-content">
                ...
                <ul>
                    ${(week.daily_tasks || [])
                        .map((task) => `<li data-task-id="${task.task_id || ''}">${task.title || "Task"}</li>`)
                        .join("")}
                </ul>
            </div>
            <div class="number">
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (result.status === "success") {
                        ...
                        // Update the stored roadmap data
                        updateLocalRoadmapTaskStatus(taskId, true);
                    }
                } else {
...
function updateLocalRoadmapTaskStatus(taskId, completed) {
    // Update the local roadmap data to mark this task as completed
    ...
    if (roadmapData.roadmap) {
        ...
        // Save updated roadmap back to localStorage
        localStorage.setItem("roadmapData", JSON.stringify(roadmapData));
...
function updateProgressDisplay(result) {
    ...
    if (progressInfo && result.progress) {
        const roadmapData = JSON.parse(localStorage.getItem("roadmapData") || "{}");
        const totalMonths = roadmapData.roadmap ? roadmapData.roadmap.length : 0;
        progressInfo.textContent = `Month ${result.progress.current_month || 1} of ${totalMonths} • Keep going`;
    }
}
